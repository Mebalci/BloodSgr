...
class VeriAc extends StatefulWidget {
  final Kullanici? profil;
  const VeriAc({super.key, this.profil});

  @override
  State<VeriAc> createState() => _VeriState();
}

class _VeriState extends State<VeriAc> {
  @override
  void initState() {
    super.initState();
    ...
    return Scaffold(
      ...
      body: BlocBuilder<VeriCubit, List<KanSekeriOlcumu>>(
        builder: (context, kisilerListesi) {
          List<KanSekeriOlcumu> acListesi = kisilerListesi.where((kisi) => kisi.acTokDurumu == 0).toList();
          if (acListesi.isNotEmpty) {
                            ...
                            IconButton(
                              onPressed: () {
                                ScaffoldMessenger.of(context).showSnackBar(
                                  SnackBar(
                                    content: Text("${kisi.kanSekeri} mg/dl silinsin mi?"),
                                    action: SnackBarAction(
                                      label: "Evet",
                                      onPressed: () {
                                        context.read<VeriCubit>().sil(kisi.id!, kisi.kullaniciId);
                                      },
                                    ),
                                  ),
                                );
            ...
            return const Center(child: Text("Açlık verisi bulunmamaktadır."));
          }
        },
      ),
    );
